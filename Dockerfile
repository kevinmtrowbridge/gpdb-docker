...
RUN echo root:pivotal | chpasswd \
        ...
        && su gpadmin -l -c "export MASTER_DATA_DIRECTORY=/gpdata/master/gpseg-1;source /usr/local/greenplum-db/greenplum_path.sh;psql -d template1 -c \"alter user gpadmin password 'secret'\"; createdb gpadmin;  exit 0"
...
#       && sysctl -p \
        ...
        && sleep 31622400 # HACK: it's difficult to get Docker to attach to the GPDB process(es) ... so, instead attach to process "sleep for 1 year"
